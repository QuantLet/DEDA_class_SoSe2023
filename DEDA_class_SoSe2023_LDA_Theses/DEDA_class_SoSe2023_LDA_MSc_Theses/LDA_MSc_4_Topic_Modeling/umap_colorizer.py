...
class UMAPColorizer:
    '''
    Custom interface designed to color the terms and theses on UMAP graphs according to their most dominant topic
    
    Args:
    
        model: An LDA model that contains document-topic and topic-term weight matrices (Already generated by LDA as the best model)
        words: A pandas dataframe containing words that were embedded in the same order as the embedding (Already generated by CorpusMaker)
        topics_names: A list of all topic titles in the same order as the topic numbers
        umap_terms_embedding: A UMAP embedding of term vectors from Word2Vec (Already generated by CorpusMaker) 
        umap_theses_embedding: A UMAP embedding of theses vectors from TF-IDF (Already generated by CorpusMaker)
        
    How to use:
        
        UMAP_col = UMAPColorizer(model, words, topics_names, umap_terms_embedding, umap_theses_embedding) <-- initializes the class
        UMAP_col.color_term_umap() <-- plots a UMAP projection for terms colored by their most dominant topic from the LDA model 
        UMAP_col.color_term_umap_subplots(rows, columns, figsize_) <-- plots separate subplots of UMAP for terms by most dominant topics
        UMAP_col.color_term_umap_slicer() <-- creates an interactive version of colored UMAP terms subplots with a slicer and hover labels
        UMAP_col.save_color_term_umap_slicer() <-- saves the interactive version of colored UMAP terms subplots as HTML
        UMAP_col.color_theses_umap(corpus, sorted_thesis_info) <-- plots a UMAP projection for theses colored by their most dominant topic from the LDA model
    '''
    ...
    def __init__(self, model, words, topics_names, umap_terms_embedding, umap_theses_embedding):
        ...
        #calculates the most dominated topics for words
    ...
    def color_term_umap(self):
        '''
        Visualizes UMAP word embeddings of a topic model with terms colored by their most dominant topics. Generates and saves both static and interactive plots for exploration.
        '''
        reset_output()
        output_notebook(resources=INLINE)
        ...
        plt.savefig('4_UMAP_terms_colored.png', transparent = True, dpi = 300)
        plt.show()
        plt.close()
        ...
        show(fig)
        output_file("UMAP words interactive colored.html")  # Set the output file path
        save(fig)
        push_notebook()
    
    
    def color_term_umap_subplots(self, rows, columns, figsize_, ):
        '''
        Generates subplots of UMAP projections for word embeddings, with each subplot representing a topic from the topic model. The terms are colored based on their association with the corresponding topics. The output is saved as an PNG-file.
        
        Args:
            rows (int): Number of rows in the grid for subplots.
            columns (int): Number of columns in the grid for subplots.
            figsize_ (tuple): Figure size in inches for the entire plot.
        '''
    ...
    def color_term_umap_slicer(self):
        '''
        Generates an interactive plot of UMAP projections for word embeddings, with each data point (term) colored based on its association with a specific topic from the topic model. The plot allows users to use a slider to select a topic, and the corresponding terms' embeddings are displayed with different colors (chosen topic vs. the rest). Hovering over the blue dots on the plot reveals the associated word for that data point.
        '''
    ...
    def save_color_term_umap_slicer(self):
        '''
        Similar to .color_term_umap_slicer() method. Saves all the interactive plots of UMAP word embeddings for each topic as HTML-pages in a "UMAP Terms by Topic" folder and creates an HTML-page that combines all the interactive plots. 
        '''
        reset_output()
        ...
        for topic in range(1, self.n_topics+1):
            ...
            # Show the plot inline and update it when the slider is changed
            #show(p, notebook_handle=True)
            output_file(os.path.join(folder_name, f"UMAP words colored topic {topic}.html"))  # Set the output file path
            save(p)
            push_notebook()
        ...
        <body>
            ...
            <div class="iframe-container">
                <iframe id="topic_iframe" src="UMAP%20Terms%20by%20Topic/UMAP%20words%20colored%20topic%201.html"></iframe>
            </div>
            ...
            <script>
                ...
                slider.oninput = function() {{
                    ...
                    iframe.src = `UMAP%20Terms%20by%20Topic/UMAP%20words%20colored%20topic%20${{topic}}.html`;
                }};
            </script>
        </body>
        </html>
        ...
        with open('UMAP terms subplots interactive colored.html', 'w') as file:
            file.write(html_content)
            
            
    def color_theses_umap(self, corpus, sorted_thesis_info):
        '''
        Visualizes UMAP thesis embeddings of a topic model with theses colored by their most dominant topics. Generates and saves both static and interactive plots for exploration.
        
        Args:
            corpus: A bag of words corpus (Already generated by CorpusMaker)
            sorted_thesis_info: A chronolically ordered dictionary with topic titles and authors (Already generated by CorpusMaker)
        '''
        reset_output()
        output_notebook(resources=INLINE)
        ...
        plt.savefig('6_UMAP_theses_colored.png', transparent = True, dpi = 300)
        plt.show()
        plt.close()
        ...
        show(fig2)
        output_file("UMAP theses interactive colored.html")  # Set the output file path
        save(fig2)
        push_notebook()