...
class UMAPColourizer:
    '''
    Custom interface designed to colour the terms and theses on UMAP graphs according to their most dominant topic
    ...
    How to use:
        ...
        UMAP_col = UMAPColourizer(model, words, topics_names, umap_terms_embedding, umap_theses_embedding) <-- initializes the class
        UMAP_col.colour_term_umap() <-- plots a UMAP projection for terms coloured by their most dominant topic from the LDA model 
        UMAP_col.colour_term_umap_subplots(rows, columns, figsize_) <-- plots separate subplots of UMAP for terms by most dominant topics
        UMAP_col.colour_term_umap_slicer() <-- creates an interactive version of coloured UMAP terms subplots with a slicer and hover labels
        UMAP_col.save_colour_term_umap_slicer() <-- saves the interactive version of coloured UMAP terms subplots as HTML
        UMAP_col.colour_theses_umap(corpus, sorted_thesis_info) <-- plots a UMAP projection for theses coloured by their most dominant topic from the LDA model
    '''
    ...
    def colour_term_umap(self):
        '''
        Visualizes UMAP word embeddings of a topic model with terms coloured by their most dominant topics. Generates and saves both static and interactive plots for exploration.
        '''
        reset_output()
        output_notebook(resources=INLINE)
    ...
    def colour_term_umap_subplots(self, rows, columns, figsize_, ):
        '''
        Generates subplots of UMAP projections for word embeddings, with each subplot representing a topic from the topic model. The terms are coloured based on their association with the corresponding topics. The output is saved as an PNG-file.
    ...
    def save_colour_term_umap_slicer(self):
        '''
        Similar to .colour_term_umap_slicer() method. Saves all the interactive plots of UMAP word embeddings for each topic as HTML-pages in a "UMAP Terms by Topic" folder and creates an HTML-page that combines all the interactive plots. 
        '''
        reset_output()
    ...
    def colour_theses_umap(self, corpus, sorted_thesis_info):
        '''
        Visualizes UMAP thesis embeddings of a topic model with theses coloured by their most dominant topics. Generates and saves both static and interactive plots for exploration.
